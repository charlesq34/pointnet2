...
parser.add_argument('--visu', action='store_true', help='Whether to dump image for error case')
parser.add_argument('--random_pc_order', action='store_true', help='Whether to use random order of points (otherwise use farthest points)')
...
TRAIN_FILES = [os.path.join(ROOT_DIR,p) for p in TRAIN_FILES]
TEST_FILES = provider.getDataFiles(\
    os.path.join(ROOT_DIR, 'data/modelnet40_ply_hdf5_2048/test_files.txt'))
TEST_FILES = [os.path.join(ROOT_DIR,p) for p in TEST_FILES]
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            batch_pred_max = np.ones((cur_batch_size, NUM_CLASSES))*(-999999) # score for classes
            batch_pred_classes = np.zeros((cur_batch_size, NUM_CLASSES)) # 0/1 for classes
            for vote_idx in range(num_votes):
                # Shuffle point order to achieve different farthest samplings
                shuffled_indices = np.arange(NUM_POINT)
                np.random.shuffle(shuffled_indices)
                rotated_data = provider.rotate_point_cloud_by_angle(current_data[start_idx:end_idx, shuffled_indices, :],
